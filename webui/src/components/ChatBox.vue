<template>
    <div class="col-8">
        <div class="container-fluid" v-if="selectedChat">

            <!-- Chat Header -->
            <div class="row">
                <div class="col-12">
                    <div class="card chat-header d-flex justify-content-between align-items-center p-3">
                        <h5 class="mb-0">{{ selectedChat.Chat_name.split("-")[1] }}</h5>
                        <button class="btn btn-light btn-sm" @click="showInfoModal = true">Info</button>
                    </div>
                </div>
            </div>

            <!-- Chat Messages -->
            <div class="row">
                <div class="col-12">
                    <div class="card chat-box">
                        <div class="card-body chat-messages">
                            <p v-if="messages.length === 0" class="text-muted">No messages yet...</p>
                            <Message v-for="(msg, _) in messages" :key="msg.message_id"  :msg="msg"
                                 />
                                 <!--@reply="handleReply" @forward="handleForward" @reaction-added="handleReactionAdded"-->
                        </div>
                    </div>
                </div>
            </div>

                <!-- Input Field -->
                <div class="row">
                    <div class="col-12">
                        <div class="input-group chat-input">
                            <input type="text" v-model="newMessage" class="form-control" placeholder="Type a message..."
                                @keyup.enter="sendMessage" />
                            <span class="input-group-text send-btn" @click="sendMessage">Send</span>
                        </div>
                    </div>
                </div>

                <!-- Info Modal -->
                <div v-if="showInfoModal" class="overlay">
                    <div class="modal-content">
                        <h5>Edit Chat Info</h5>
                        <input v-model="editableChatName" class="form-control mb-2" placeholder="Chat Name" />

                        <h6>Members:</h6>
                        <ul class="list-group mb-2">
                            <li v-for="(member, index) in selectedChat.members" :key="index" class="list-group-item">
                                {{ member }}
                            </li>
                        </ul>

                        <div class="input-group mb-2">
                            <input v-model="newMember" type="text" class="form-control" placeholder="Add a member..." />
                            <button class="btn btn-primary" @click="addMember">+</button>
                        </div>

                        <div class="input-group mb-2">
                            <button class="btn btn-danger" @click="showInfoModal = false">Leave chat</button>
                        </div>

                        <div class="d-flex justify-content-between">
                            <button class="btn btn-danger" @click="showInfoModal = false">Close</button>
                            <button class="btn btn-success" @click="saveChatName">Save</button>
                        </div>
                    </div>
                </div>

            </div>
        </div>
</template>

<script>
export default {
    props: ["selectedChat", "identifier", "messages"],

    data() {
        return {
            //messages: [],
            newMessage: "",
            showInfoModal: false,
            editableChatName: "",
            newMember: ""
        };
    },
    watch: {
        selectedChat(newChat) {
            if (newChat) {
                this.editableChatName = newChat.Chat_name;
            }
        }
    },
    methods: {
        async sendMessage() {
            if (this.newMessage.trim() !== "") {
                try {
                    let response = await this.$axios.post("/chats/" + this.chat_id + "/messages", {
                        chat_id: this.selectedChat.first_chat_id,
                        content: this.newMessage,
                        forwarded: 0,
                        reply: 0,
                    });

                    this.newMessage = ""
                    //console.log(response)
                    this.loadMessage()
                }
                catch (error) {
                    console.log(error)
                }
            }
        },

        async loadMessage() {
            try {
                let response = await this.$axios.get("/users/" + this.identifier + "/chats/" + this.selectedChat.first_chat_id, {
                });

                this.messages = response.data.messages
            }
            catch (error) {
                console.log(error)
            }
        },


        addMember() {
            if (this.newMember.trim() !== "") {
                this.selectedChat.members.push(this.newMember);
                this.newMember = "";
            }
        },
        saveChatName() {
            if (this.editableChatName.trim() !== "") {
                this.selectedChat.Chat_name = this.editableChatName;
            }
            this.showInfoModal = false;
        }
    },

    async mounted() {
        await this.loadMessage();
    }
};
</script>

<style scoped>
/* Chat Header */
.chat-header {
    background-color: #008069;
    color: white;
    border-radius: 10px;
}

/* Chat Box */
.chat-box {
    height: 400px;
    overflow-y: auto;
    background-color: #f5f5f5;
    padding: 10px;
    border-radius: 10px;
}

/* Messages */
.chat-messages {
    display: flex;
    flex-direction: column;
    gap: 10px;
}

.message {
    background: #dcf8c6;
    padding: 10px;
    border-radius: 10px;
    width: fit-content;
    max-width: 70%;
}

/* Input Field */
.chat-input {
    margin-top: 10px;
}

.send-btn {
    cursor: pointer;
    background-color: #008069;
    color: white;
    border: none;
}

.send-btn:hover {
    background-color: #006a58;
}

/* Overlay Modal */
.overlay {
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background: rgba(0, 0, 0, 0.5);
    display: flex;
    justify-content: center;
    align-items: center;
}

.modal-content {
    background: white;
    padding: 20px;
    border-radius: 10px;
    width: 300px;
    box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.3);
}
</style>